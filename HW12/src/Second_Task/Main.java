...
public class Main {
	...
	public static void main(String[] args) {
		...
		List <Month> arr = new ArrayList<Month>();
		
		EnumSet <Month> all = EnumSet.allOf( Month.class);
		List <Month> list = new ArrayList<>( all.size());
		for (Month m : all) {
		    arr.add(m);
		}
	
		List <Season> sarr = new ArrayList<Season>();
		
		EnumSet <Season> sall = EnumSet.allOf( Season.class);
		List <Season> slist = new ArrayList<>( sall.size());
		for (Season s : sall) {
		    sarr.add(s);
		}
